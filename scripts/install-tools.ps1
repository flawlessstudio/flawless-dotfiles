...
function Ensure-Mise {
  ...
  if (-not $installed) {
    throw "mise was installed but cannot be resolved in this PowerShell session."
  }
...
if ($LASTEXITCODE -eq 1) {
  throw "mise still reports configured tools missing after install."
}
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

Write-Host "PASS  pinned toolchain converged"
exit 0
