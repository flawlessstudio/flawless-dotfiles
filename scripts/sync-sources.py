#!/usr/bin/env python3
"""Safely materialize pinned external SSOT repositories.
...
- never pushes;
- refuses dirty repositories;
- refuses origin mismatches;
...
def load_manifest(path: Path) -> list[dict[str, Any]]:
    ...
    required = {"id", "repository", "clone_url", "branch", "commit", "destination", "role", "required"}
    ...
    for index, source in enumerate(data["sources"]):
        ...
        if not ID.fullmatch(str(source["id"])):
            raise SystemExit(f"invalid source id: {source['id']!r}")
        if not SHA40.fullmatch(str(source["commit"])):
            raise SystemExit(f"invalid 40-character commit pin for {source['id']}")
        destination = Path(str(source["destination"]))
        if destination.is_absolute() or ".." in destination.parts:
            raise SystemExit(f"unsafe destination for {source['id']}: {destination}")
...
def apply_source(source: dict[str, Any], target: Path) -> None:
    ...
    if target.exists():
        ...
        if clone.returncode != 0:
            ...
            if source.get("private"):
                hint = " Authenticate Git for this private source (for example with GitHub CLI) and retry."
    ...
    if fetch.returncode != 0:
        hint = ""
        if source.get("private"):
            hint = " Check GitHub authentication/authorization."
